...
function FilteredHLConsumer(options) {
    if (this instanceof FilteredHLConsumer === false) {
        return new FilteredHLConsumer(options);
    }
    ...
    this.pause = function (cb) {
        this.isPaused = true;
        this.hlConsumer.pause(cb);
    };

    this.resume = function (cb) {
        this.isPaused = false;
        this.hlConsumer.resume(cb);
    };
    ...
    function _createConsumer(cb) {
        ...
        java.newInstance("com.liveperson.kafka.consumer.TopicFilterHLConsumer", zookeeperUrl, groupId, topic, properties, serverPort, threadCount, exceptionListener, function (err, hlConsumer) {
            if (!err) {
...
util.inherits(FilteredHLConsumer, events.EventEmitter);

module.exports = FilteredHLConsumer;
